    ...
    for CV_val in Kfold_number:
        ...
        for model in models:
            ...
            df_model_values = df_model_values.append({'CV':CV_val,'Model':str(model_name),
                                                      'Model_Accuracy':round((np.mean(scores)),3),
                                                      'Model_STD':round((np.std(scores)),3), 
                                                      'Accuracy_Score': round(accuracy_train,3)}, ignore_index = True)
    df_model_values.sort_values(by=['Model_Accuracy'], axis=0, ascending=False, 
